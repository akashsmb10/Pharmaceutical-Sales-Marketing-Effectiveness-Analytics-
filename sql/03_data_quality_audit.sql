...
SELECT 'duplicate_physician_month' AS check_name, physician_id, month_start, COUNT(*) AS n FROM prescription_month GROUP BY physician_id,month_start HAVING COUNT(*)>1;
...
SELECT 'invalid_prescription_value' AS check_name, physician_id, month_start FROM prescription_month WHERE prescriptions<0 OR revenue_usd<0 OR target_rx<=0 OR month_start IS NULL;
...
SELECT 'row_counts' AS check_name,(SELECT COUNT(*) FROM territory) territories,(SELECT COUNT(*) FROM physician) physicians,(SELECT COUNT(*) FROM campaign) campaigns,(SELECT COUNT(*) FROM prescription_month) physician_months,(SELECT COUNT(*) FROM outreach) contacts;
